	...
	for _, honk := range honks {
		...
		for _, d := range honk.Donks {
			...
			if strings.HasPrefix(d.Media, "image") {
				...
				s := `<video loop="true" autoplay="true" muted="true" style="max-width: 100%;height: auto;"><source src="%s" type="video/mp4"></video>`;
...
func autoavatate(r *http.Request, n string, w http.ResponseWriter) {
	a := genAvatar(n)
	if !develMode {
